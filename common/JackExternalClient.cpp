...
int JackExternalClient::Open(const char* name, int pid, int origlen, int refnum, int uuid, int* shared_client)
{
    try {
        ...
        shared_mem->Init();
        fClientControl = new(shared_mem) JackClientControl(name, pid, origlen, refnum, uuid);
